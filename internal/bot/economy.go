...
func (b *Bot) cmdDaily(s *discordgo.Session, m *discordgo.MessageCreate) {
	if m.GuildID == "" {
		return
	}
	...
	reward := 100
	err = b.store.UpdateUserDaily(context.Background(), m.Author.ID, m.GuildID, user.Balance+reward, now.Format(time.RFC3339))
	if err != nil {
		return
	}
...
func (b *Bot) cmdBalance(s *discordgo.Session, m *discordgo.MessageCreate) {
	if m.GuildID == "" {
		return
	}
	...
	user, err := b.store.GetUser(context.Background(), target.ID, m.GuildID)
	if err != nil {
		return
	}
...
func (b *Bot) cmdGive(s *discordgo.Session, m *discordgo.MessageCreate, args []string) {
	if m.GuildID == "" {
		return
	}
	...
	sender, err := b.store.GetUser(context.Background(), m.Author.ID, m.GuildID)
	if err != nil {
		return
	}
	...
	receiver, err := b.store.GetUser(context.Background(), target.ID, m.GuildID)
	if err != nil {
		return
	}

	b.store.UpdateUserBalance(context.Background(), m.Author.ID, m.GuildID, sender.Balance-amount)
	b.store.UpdateUserBalance(context.Background(), target.ID, m.GuildID, receiver.Balance+amount)
