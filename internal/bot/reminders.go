...
func (b *Bot) LoadReminders(s *discordgo.Session) {
	reminders, err := b.store.GetPendingReminders(context.Background())
	if err != nil {
...
func (b *Bot) scheduleReminder(s *discordgo.Session, id int, userID, message string, delay time.Duration) {
	time.AfterFunc(delay, func() {
		b.fireReminder(s, id, userID, message)
	})
}

func (b *Bot) fireReminder(s *discordgo.Session, id int, userID, message string) {
	_ = b.store.DeleteReminder(context.Background(), id)
...
func (b *Bot) cmdRemind(s *discordgo.Session, m *discordgo.MessageCreate, args []string) {
	if len(args) < 3 {
	...
	id, err := b.store.CreateReminder(context.Background(), m.Author.ID, m.ChannelID, guildID, message, remindAt)
	if err != nil {
	...
	b.scheduleReminder(s, id, m.Author.ID, message, d)
