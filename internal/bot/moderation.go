...
func (b *Bot) cmdKick(s *discordgo.Session, m *discordgo.MessageCreate, args []string) {
	if m.GuildID == "" {
...
func (b *Bot) cmdBan(s *discordgo.Session, m *discordgo.MessageCreate, args []string) {
	if m.GuildID == "" {
		embed := newEmbed("❌ Error", "This command can only be used in a server.")
		SendEmbed(s, m.ChannelID, embed)
		return
...
func (b *Bot) cmdMute(s *discordgo.Session, m *discordgo.MessageCreate, _ []string) {
	if m.GuildID == "" {
		embed := newEmbed("❌ Error", "This command can only be used in a server.")
		SendEmbed(s, m.ChannelID, embed)
		return
