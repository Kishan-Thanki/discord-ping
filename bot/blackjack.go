...
var activeBlackjackGames = sync.Map{}

func cmdBlackjack(s *discordgo.Session, m *discordgo.MessageCreate, args []string) {
	if m.GuildID == "" {
		return
	}

	if _, active := activeBlackjackGames.Load(m.Author.ID); active {
	...
	_ = database.UpdateUserBalance(m.Author.ID, m.GuildID, user.Balance-bet)
	...
	if err == nil {
		...
		activeBlackjackGames.Store(m.Author.ID, game)

		playerTotal := calculateHand(game.PlayerHand)
		if playerTotal == 21 {
			game.mu.Lock()
			resolveBlackjackGame(s, game)
			game.mu.Unlock()
		}
	} else {
...
func handleBlackjackInteraction(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	val, exists := activeBlackjackGames.Load(i.Member.User.ID)
	if !exists {
...
func resolveBlackjackGame(s *discordgo.Session, game *BlackjackGame) {
	...
	if payout > 0 {
		user, _ := database.GetUser(game.UserID, game.GuildID)
		_ = database.UpdateUserBalance(game.UserID, game.GuildID, user.Balance+payout)
	}
