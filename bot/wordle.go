...
var activeWordleGames = sync.Map{}

func cmdWordle(s *discordgo.Session, m *discordgo.MessageCreate) {
	if m.GuildID == "" {
		return
	}

	if _, exists := activeWordleGames.Load(m.Author.ID); exists {
	...
	activeWordleGames.Store(m.Author.ID, game)

	embed := newEmbed("🟩 Discord Wordle", "A new game has started!\nUse `!guess <5-letter-word>` to play.\n\n⬜ ⬜ ⬜ ⬜ ⬜\n⬜ ⬜ ⬜ ⬜ ⬜\n⬜ ⬜ ⬜ ⬜ ⬜\n⬜ ⬜ ⬜ ⬜ ⬜\n⬜ ⬜ ⬜ ⬜ ⬜\n⬜ ⬜ ⬜ ⬜ ⬜")
	SendEmbed(s, m.ChannelID, embed)
}

func cmdGuess(s *discordgo.Session, m *discordgo.MessageCreate, args []string) {
	if m.GuildID == "" {
		return
	}

	val, exists := activeWordleGames.Load(m.Author.ID)
	if !exists {
	...
	if game.Finished {
		activeWordleGames.Delete(m.Author.ID)

		if won {
			payouts := []int{1000, 500, 250, 100, 50, 25}
			reward := payouts[len(game.Guesses)-1]

			user, _ := database.GetUser(m.Author.ID, m.GuildID)
			_ = database.UpdateUserBalance(m.Author.ID, m.GuildID, user.Balance+reward)
